...
/** ast搜索转换 */
export function transformBySearch(source:string,conf:{
  folder: string
  Dictionary:TDictionary
  regExp:string
  ...
  const {folder,Dictionary,regExp,isWrap} = conf;
  let re = source;
...
export function getKeyformStringLiteral(json:Object,value:string | number){
  for (const [key,val] of Object.entries(json)) {
...
/** ast 转换对象属性 */
export function transformStringLiteral(source:string | number,conf:{
  folder: string
  Dictionary:TDictionary
}){
  const {folder,Dictionary} = conf;
  let re = '';
   ...
   for (let i = 0; i < contentValues.length; i++) {
    const getRet = getKeyformStringLiteral(contentValues[i],source);
    if(getRet !== undefined){
      re = getRet;
      break;
    }
